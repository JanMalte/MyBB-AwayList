<?php
/**
* This is a Plugin for the MyBB
* Coded by Jan Malte Gerth
* Please keep the backlink on the bottom
*
	"name"			=> "Aufenthaltsliste",
	"description"	=> "Zeigt wann wer in einem bestimmten Land ist",
	"website"		=> "http://mybb.malte-gerth.de/",
...
/**********************************************************************
*
* needed additional functions for this plugin
*
*/
...
function show_months($P_name, $P_month = false)
{
	...
	if($P_month == false) $P_month = date("m");
	// set the start HTML of the select form
	$html_select_form = "<select name=\"{$P_name}\">";
...
{
	if($PARAM_former_date_day == false) $PARAM_former_date_day = date("d");
	if($PARAM_former_date_month == false) $PARAM_former_date_month = date("m");
	if($PARAM_former_date_year == false) $PARAM_former_date_year = date("y");
...
function check_future_date($P_first_date, $P_second_date=false)
{
	...
	if($P_second_date == false) $P_second_date = time();
...
function show_form_new_data()
{
	...
	if($mybb->user['uid'] == 0) $errors[] = "Sie sind nicht angemeldet. Bitt melden Sie sich an bevor Sie sich in die Liste eintragen.";
...
function show_form_edit_data()
{
	...
	// check the userrights
	if($mybb->user['uid'] == 0) $errors[] = "Sie sind nicht angemeldet. Bitt melden Sie sich an bevor Sie Einträge ändern.";
	...
	if($_GET['data_id'] == '') $errors[] = "Es wurde kein Datensatz zum Bearbeiten ausgewählt.";
...
function insert_new_data()
{
	...
	if($_POST['airline'] == "") $errors[] = "Bitte geben Sie eine Airline ein.";
	if($_POST['ort'] == "") $errors[] = "Bitte geben Sie einen Urlaubsort ein.";
	if($_POST['hotel'] == "") $errosr[] = "Bitte geben Sie einen Hotelnamen ein.";
	...
	if($check == false ) $errors[] = "Sie sind zu diesem Zeitpunkt schon unterwegs";
	// Ankunft (Hinflug)
	if(!check_future_date($ankunft)) $errors[] = "Das Ankunftsdatum liegt nicht in der Zukunft.";
	// Abflug (Rückflug)
	if(!check_future_date($abflug)) $errors[] = "Das Abflugsdatum liegt nicht in der Zukunft.";
	if(!check_future_date($abflug,$ankunft)) $errors[] = "Das Abflugsdatum liegt vor dem Ankunftsdatum";
...
function edit_data()
{
	...
	// get any other errors
	if($_POST['data_id'] == "") $errors[] = "Es wurde kein Datensatz gewählt.";
	if($_POST['airline'] == "") $errors[] = "Bitte geben Sie eine Airline ein.";
	if($_POST['ort'] == "") $errors[] = "Bitte geben Sie einen Urlaubsort ein.";
	if($_POST['hotel'] == "") $errosr[] = "Bitte geben Sie einen Hotelnamen ein.";
	if(preg_match("/^[0-9[:space:]]*$/",$_POST['telefon'],$number))
...
function delete_data_notification()
{
	...
	// check the userrights
	if($mybb->user['uid'] == 0) $errors[] = "Sie sind nicht angemeldet. Bitt melden Sie sich an bevor Sie Einträge löschen.";
	...
	// get any other errors
	if($_GET['data_id'] == "") $errors[] = "Es wurde kein Datensatz gewählt.";
...
function show_dates($message = '', $action = false, $limit = 20, $moment = 0, $offset = 0)
{
	...
	if($get_delete_query) {
		...
		if($count_users == 0) $count_users = 'keine';
	}
...
function liste_activate()
{
	...
	if($dbversion > 5)
	{
		$query = "CREATE TABLE IF NOT EXISTS `".$db->table_prefix."liste` (
...
function liste_deactivate()
{
	...
	find_replace_templatesets("index", '#<br />(\r?)(\n?){\$liste}<br />(\r?)(\n?)#', "\r\n", 0);
